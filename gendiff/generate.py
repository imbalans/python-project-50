    ...
    for key in keys:
        if key in content1 and key in content2:
            if isinstance(content1[key], dict) and isinstance(content2[key], dict):
                result.append(
                    {"type": "not updated", "name_key": key, "value": build_diff(content1[key], content2[key])})
                ...
                result.append(
                    {"type": "updated", "name_key": key, "new_value": build_diff(content2[key], content2[key]),
            ...
            if isinstance(content1[key], dict):
                result.append(
                    {"type": "removed", "name_key": key, "old_value": build_diff(content1[key], content1[key])})
            else:
                result.append({"type": "removed", "name_key": key, "old_value": content1[key]})
        else:
            if isinstance(content2[key], dict):
                result.append(
                    {"type": "added", "name_key": key, "new_value": build_diff(content2[key], content2[key])})
            else:
